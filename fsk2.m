...
t3 = t2(1:8*length(t1));
m3 = mod_Signal(1:8*length(t1));

% frequency deivation
instantaneous_phase = unwrap(angle(hilbert(m3)));

% Compute the frequency deviation
frequency_deviation = diff(instantaneous_phase) / (2 * pi * (t3(2) - t3(1)));

% Plot the frequency deviation
figure; figure;
figure(3); % Create a new figure for the frequency deviation plot
plot(t3(2:end), frequency_deviation);
